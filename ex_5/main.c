...
static void responseTask(void* args){
	...
	while(1){
		if (!gpio_get_pin_value(a.pin.test)){
			if(a.delay){
...
int main(){
	...
	xTaskCreate(responseTask, "", 1024, (&(struct responseTaskArgs){{TEST_A, RESPONSE_A}, 0}), configMAX_PRIORITIES, NULL);
	xTaskCreate(responseTask, "", 1024, (&(struct responseTaskArgs){{TEST_B, RESPONSE_B}, 0}), configMAX_PRIORITIES, NULL);
